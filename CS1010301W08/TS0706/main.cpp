...
{   
    if(cur.x<0||cur.x>MAX+1||cur.y<0||cur.y>MAX+1||turns>TURN_MAX)return false;
...
bool isSameTile(Pos cur,Pos target)
{
...
int main()
{
    ...
    while( cin>>curX>>curY>>targetX>>targetY)
    {
        ...
        target.x = targetX+1;
        target.y = targetY+1;
        if(isSameTile(cur,target)&&isValid(cur,target,0,5))
        {
            deleTile(cur,target);
            cout<<"pair matched";
        }
